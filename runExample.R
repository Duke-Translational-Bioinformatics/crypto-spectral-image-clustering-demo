## Crypto Image Spectral Clustering
...
runExample <- function(... = NULL){
  ...
  ## LOAD DATA OBJECTS
  cat('Loading Cryptococcus image from Synapse\n')
  imageEnt <- synGet('syn2636223')
  imageFilePath <- imageEnt@filePath
  
  rawimg=readJPEG(imageFilePath)
  rawimg=t(as.matrix(as.data.frame(rawimg)[ , 1:180]))
  ...
  kmL=kmeans(evL$vectors[,(ncol(simmatrix)-1):(ncol(simmatrix)-0)],centers=8,nstart=5)
  segmatL=matrix(kmL$cluster-1,newdim[1],newdim[2],byrow=T)
  if(max(segmatL) & sum(segmatL==1)<sum(segmatL==0)) {segmatL=abs(segmatL-1)}
  
  kmU=kmeans(evU$vectors[,(ncol(simmatrix)-1):(ncol(simmatrix)-0)],centers=8,nstart=5)
